...
const { Domain, User, Post, Hashtag } = require('../models'); // Domain과 User 모델 불러오기
...
exports.tokenTest = (req, res) => {
    req.json(res.locals.decoded); // 미들웨어에서 검증된 토큰의 decoded 정보를 반환
};
...
exports.getPostsByHashtag = async (req, res) => {
    try {
        ...
        if (!hashtag) { // 해시태그가 존재하지 않는 경우
            return res.status(404).josn({ // 404 Not Found 응답
                code: 404,
